...
import { generateSeoKeywords, SeoKeywordGeneratorInput, SeoKeywordGeneratorOutput } from '@/ai/flows/seo-keyword-generator';
    ...
    <Card className="shadow-xl border border-border/50">
      <CardHeader>
        <CardTitle className="flex items-center text-2xl text-accent font-headline">
          <Wand2 className="mr-2 h-6 w-6" /> SEO Keyword Generator
        </CardTitle>
        <CardDescription>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <Label htmlFor="portfolioContent" className="text-foreground/80">Portfolio Content</Label>
            <Textarea
              id="portfolioContent"
              value={portfolioContent}
              onChange={(e) => setPortfolioContent(e.target.value)}
              placeholder="Describe your skills, projects, and services..."
              rows={8}
              className="mt-1 bg-card border-border/70 focus:border-accent"
              disabled={isLoading}
            />
          </div>
          <Button type="submit" disabled={isLoading} className="w-full md:w-auto bg-primary text-primary-foreground hover:bg-primary/90 transition-transform hover:scale-105">
            {isLoading ? (
              <>
      ...
      {keywords.length > 0 && (
        <CardFooter className="flex-col items-start space-y-4 pt-6 border-t border-border/50">
            <div className="flex justify-between items-center w-full">
              <h4 className="text-lg font-semibold text-foreground">Generated Keywords:</h4>
              <Button onClick={handleCopy} variant="ghost" size="sm" className="text-accent hover:text-accent/80">
            ...
            <div className="flex flex-wrap gap-2">
              {keywords.map((keyword, index) => (
                <Badge key={index} variant="outline" className="px-3 py-1 text-sm border-accent/50 text-accent bg-accent/10">
                  {keyword}
                </Badge>
              ))}
            </div>
